...
import { selectAllProducts } from "@/redux/Selectors/dashboardSelectors"; // تأكد من مسار السيلكتور بتاعك
import { Plus, Search, Filter, Loader2, AlertCircle } from "lucide-react";
import { useEffect } from "react";

export default function ProductsPage() {
  // 1. تشغيل الهوك المسؤول عن الـ Request وعمل الـ Subscription في الكاش
  // const { data,isLoading, isError, error ,refetch} = useGetProductsQuery('');
  const res = useGetProductsQuery('');

  console.log(res);
  // 2. استخدام السيلكتور الصافي لجلب المنتجات من الكاش مباشرة بعد التحديث
  ...
  return (
    ...
    //         <p className="text-xs text-muted-foreground">جاري جلب المنتجات من السيرفر...</p>
    //       </div>
    //     )}
    ...
    //           {error && 'status' in error ? `خطأ: ${error.status}` : 'حدث خطأ غير متوقع'}
    //         </p>
    //       </div>
    //     )}
    ...
    //         <p className="text-xs text-muted-foreground max-w-sm">
    //           لم نجد أي منتجات مسجلة في النظام حالياً. اضغط على إضافة منتج جديد للبدء.
    //         </p>
    //       </div>
    //     )}
    ...
    //                 <td className="py-3 font-mono text-muted-foreground">#{product.id}</td>
    //                 <td className="py-3 font-semibold">{product.name || product.title}</td>
    //                 <td className="py-3">{product.price} جنيه</td>
    //                 <td className="py-3">
    //                   <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${
    //                     product.stock > 0 ? 'bg-green-500/10 text-green-500' : 'bg-destructive/10 text-destructive'
    //                   }`}>
