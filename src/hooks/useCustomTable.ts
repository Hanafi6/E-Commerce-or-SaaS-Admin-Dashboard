...
const AUTO_FILTER_COLUMNS = {
  price: { filterFn: "equals", enableSorting: true },
  stock: { filterFn: "equals", enableSorting: true },
} as const satisfies Record<
  string,
  { filterFn: ColumnDef<unknown, unknown>["filterFn"]; enableSorting?: boolean }
>;
...
interface UseCustomTableProps<TData> {
  ...
  customCells?: {
    [K in keyof TData]?: (
      info: CellContext<TData, TData[K]>,
    ) => React.ReactNode;
  };
}
...
export function useCustomTable<TData extends Record<string, any>>({
  ...
  const columns = useMemo(() => {
    ...
    const keys =
      data.length > 0 ? Object.keys(data[0]) : (columnKeys?.map(String) ?? []);

    return keys
      .filter((key) => !skipColumns.includes(key as keyof TData))
      .map((key) => {
        const filterConfig = AUTO_FILTER_COLUMNS[key as "price" | "stock"];
